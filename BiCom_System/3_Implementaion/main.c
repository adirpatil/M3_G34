 ...
 * @return flag
 */
int encrypt() // Function for encryption

{
    bool flag = false;
    const int a = 123;
    const int b = 234;
    int result = a * b; // Algorithm used for encryption
    if (result == 28782)
    {
...
/**
 * @brief Function for alarm status all LEDs are off at the same time
 *
 */
...
/**
 * @brief Function window status all LEDs are on at the same time
 *
 */
...
/**
 * @brief To show battery status the LEDs are made on in clockwise direction
 *
 */
...
/**
 * @brief To show the door status the LEDs are made on in anti-clockwise direction
 *
 */
...
int main(void)
{
    ...
    GpioLed.pGPIOx = GPIOD;                                 // The declaration of the pins
    ...
    if (encrypt())
    {
        ...
        while (1)
        {
            ...
            {
                delay();
                count = count + 1; // count is incremented when push button is pressed 
            }
            if (count == 3)
            {
                window_status();
